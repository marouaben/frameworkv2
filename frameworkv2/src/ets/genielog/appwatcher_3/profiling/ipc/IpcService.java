...
public class IpcService {
	...
	private class QueuedTask {
		 ...
		 */
		QueuedTask(ipcAction[] action, int pid, long timestamp,
	...
	public void forceExit() {
		if (Config.DEBUG_PROFILING)
			Log.d(Config.TAG_APP, TAG + "forceExit");
		if (!checkStatus())
			return;
	...
	public boolean addRequest(ipcAction[] action, int pid, int sec,
		...
		try {
			...
			QueuedTask newTask = new QueuedTask(action, pid, timestamp, obj);
	...
	public void removeRequest(ipcClientListener listener) {
		...
		try {
			QueuedTask checkObj = new QueuedTask(null, -1, 0, listener);
			cmdQueueLock.acquire();
